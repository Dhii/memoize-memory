 ...
 * @since [*next-version*]
 */
class AbstractBaseContainerMemoryTest extends TestCase
{
    /**
     ...
     * @since [*next-version*]
     */
    const TEST_SUBJECT_CLASSNAME = 'Dhii\Cache\AbstractBaseContainerMemory';
    ...
    /**
     * Tests that `get()` fails as expected when a container exception is encountered.
     *
     * @since [*next-version*]
     */
    public function testGetFailureContainer()
    {
        $key = uniqid('key');
        $containerException = $this->createContainerException('Problem getting value');
        $subject = $this->createInstance(['_getCached']);
    ...
    /**
     * Tests that `get()` fails as expected when a non-container exception is encountered.
     *
     * @since [*next-version*]
     */
    ...
    {
        $key = uniqid('key');
        $exception = $this->createException('Problem during value retrieval');
        $containerException = $this->createContainerException('Problem getting value');
        $subject = $this->createInstance(['_getCached', '_createContainerException']);
        $_subject = $this->reflect($subject);
    ...
    /**
     * Tests that `_set()` fails as expected when a container exception is encountered.
     *
     * @since [*next-version*]
     */
    public function testSetFailureInvalidArgument()
    {
        $key = uniqid('key');
        $val = uniqid('val');
        $ttl = rand(1, 999999);
        $exception = $this->createInvalidArgumentException('Invalid key');
        $subject = $this->createInstance(['_setData']);
        $_subject = $this->reflect($subject);
        ...
        $_subject->_set($key, $val, $ttl);
    }

    /**
     * Tests that `_set()` fails as expected when a non-container exception is encountered.
     *
     * @since [*next-version*]
     */
    public function testSetFailure()
    {
        ...
        $subject = $this->createInstance(['_setData', '_createContainerException']);
        $_subject = $this->reflect($subject);
